    ...
    public function import_zotero_data_handler() {
		if(
			...
			for($start = 0; true; $start+=$limit ){ $found_items = 0;
				...
				$top_api_response = wp_remote_get($top_requset_url);
			...
			for($start = 0; true; $start+=$limit ){ $found_items = 0;
				...
				$items_api_response = wp_remote_get($items_requset_url);
        	...
        	if ($handle) { $no = 0; 
        		...
        		if(!empty($notes)){ $i = 0;
        			...
        			if(empty($risMissing) && empty($dbMissing)){
        				...
        				if(!empty($dbMissing)){
        					...
        					foreach($dbMissing as $lineno => $item){
								$msg = $this->get_line_message($lineno, false);
								$msg .= " <b>$item</b> facet does not exist!";
								$errorData[] = $msg;
							}
        					$error[] = [
        			...
        			if(!empty($noNoteItems) ){
						...
						$error[] = [
    						'message' => 'Following items lacks index notes!',
    						'data' => $errorData,
    					]; 
